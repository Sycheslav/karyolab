...
- [07_аномалии_замещения_и_нестандартные_кариотипы.md](07_аномалии_замещения_и_нестандартные_кариотипы.md) - трисомии, анеуплоидии, моно- и нуллисомии, замещения, транслокации.
- [08_лицевой_кариотип_образца.md](08_лицевой_кариотип_образца.md) - выбор лучших хромосом и фиксация результата по образцу.
...
flowchart LR
    ...
    SampleKaryotype --> OverviewExport["Экспорт обзора"]
    SampleKaryotype --> Atlas["Атлас"]
```

## Навигационная Модель

Раздел `Кариотип` раскрывается на три рабочие вкладки:

- `Импорт` - загрузить PSD, выбрать окрашенный препарат, извлечь хромосомы из слоев, проверить метаданные и сохранить результат.
...
- `Экспорт` - собрать обзорные изображения и таблицы по шаблонам.

Внутри вкладки `Кариотип` есть два режима: `разметка хромосом` и `разметка генома`. Первый описывает отдельную хромосому, второй собирает картину по метафазе или гибридизации.

## Основной Принцип

Каждая картинка в кариотипе должна оставаться связанной с происхождением:

`образец -> растение или смесь растений -> препарат -> окрашенный препарат -> фото или PSD -> метафаза -> хромосома -> идеограмма -> кариотип -> обзор`.
